...
現在の共通モデルは、その後に完了したTODOと [共通API仕様](../spec.md) を参照する。
...
5. `NormalizedBook`、`BookSource`、`Identifier`、`Contributor`、`Series`、
   `Volume`、`Price` の最小仕様を決める。
6. 既存 `api.Book` は移行期間を設けず、破壊的に置き換える。
7. `docs/spec.md` へ確定した共通仕様だけを反映する。
...
新しい共通仕様の根拠としてそのまま移植しない。

電子書籍のタイトル検索には、初期実装では楽天Koboを使用する。
Yahoo!ショッピングの `ebookjapan` は商品検索APIで取得できないため採用しない。

DMM.com APIは、2026-08-07にアフィリエイト承認とアフィリエイトIDの取得を確認した。
API仕様、実レスポンス、利用条件は未調査のため、追加採用は調査後に判断する。
...
- DMMはアフィリエイト承認済みであり、API仕様と実レスポンスの調査後に追加採用を判断する。
