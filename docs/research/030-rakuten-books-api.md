...
- `author` / `authorKana` は実例で確認した `/` 区切りを人物単位へ分割する。両者の分割数が一致する場合だけReadingを同位置へ対応付け、不一致時は推測しない
...
TODO030時点の未決事項のうち、TODO031・TODO032で次を確定した。
...
- `author` / `authorKana` は `/` 区切りを人物単位へ分割し、分割数一致時だけReadingを対応付ける。氏名内部の空白・カンマ等の正規化や役割推測は行わない
