...
タイトル、著者名、主要な書誌項目で検索し、ISBNから書籍を参照して、`model.Book`
へ変換する。指定語を含む検索結果の除外にも対応する。
...
`model.Price` が表す書籍の定価または取得時点販売価格とは意味が異なるため変換しない。
所蔵情報も現在の取得対象外とする。
...
```go
func Attributions() []madb.Attribution
...
通常利用する型とエラーの定義元は `model` パッケージである。`madb` は必要な型と定数を
エイリアスとして公開するため、利用側は `madb` だけをインポートして、検索、結果の参照、
分類済みエラーの判定を行える。
...
bindingを集約してから `madb.Book` へ変換する。MADB固有の役割表記、ISBN、
欠落値の扱いはこの節で定義する。
...
先頭の角括弧内が次の表記と完全一致する場合だけ、`Contributors[].Roles` で使う役割名へ変換する。
...
複数の役割名へ変換する。1つでも未知の構成要素があれば、複合役割全体を
未知として扱う。同じ役割名になる構成要素は1件へまとめる。

`Authors` へ含める役割は `著者`、`原作`、`脚本`、`作画`、
...
元の入力位置ごとに結果を返す。該当なしの `Books` は `nil` ではない空スライスとする。
...
`Attributions()` は外部通信を行わず、次の `data` クレジットを1件返す。検索とISBN参照が成功した場合は、該当書籍が0件でも同じ値を結果の `Attributions` に設定する。
...
- `License` / `LicenseURL`: 空

クレジット文はMADBの出典と、mankenによる `Book` モデルへの変換を示す。ライセンス情報は設定せず、利用条件の確認先を `RequirementsURL` で示す。返却するクレジット情報だけで利用条件への適合を保証せず、利用時点の公式条件を確認する。
