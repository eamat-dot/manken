...
クレジット文には、mankenがデータを `Book` モデルへ変換したことも含まれます。`Book` のデータをさらに編集・加工して利用する場合は、利用側で行った加工についても記載してください。

返される情報だけで利用規約への適合が保証されるわけではありません。利用時点の公式条件も確認してください。

## 詳細仕様

検索条件、カーソル、ISBN参照、Raw response、変換、通信、エラーの完全な仕様は[manken MADBパッケージ仕様](spec.md)を参照してください。
