...
`レーベル:`をPublishers、`発売日:`をReleaseDateとして扱う。Tower漫画カテゴリの実測では `レーベル:` に
講談社、集英社、小学館等の出版社名が入るため、ラベル名だけを根拠に `PublicationSeries` や `BookSeries` へ変換しない。この規則は
Tower固定の範囲だけに適用する。`releaseDate` は公式のstring表記に対して実APIでnullやUnix秒数値を返す例があるため、
発売日は型と表記が明示された `発売日:` ラベルを優先し、取得元の精度を維持する。`アーティスト:`は役割不明の不完全な寄与者表示であり、原作者などの人物や他の寄与者を
