...
result, err := client.SearchBooks(ctx, ndl.SearchBooksRequest{Title: "動物のお医者さん"})
```
...
出版時期、件名、内容記述でさらに絞り込む場合は`SearchOptions`を使う。
...
`From` / `Until`は`YYYY`、`YYYY-MM`、`YYYY-MM-DD`で指定し、両方使う場合は精度をそろえる。`Subject`は人物・地域・題材等の件名検索、`Description`は要約等を含む内容記述系インデックスの検索に使う。`Description`で検索できた語が変換前SRU XMLに含まれるとは限らず、現在のNDL実装は「要約等」の本文を`Normalized.Description`へ格納しない。
