...
データのクレジット文には、mankenが `Book` モデルへ変換したことも含まれます。`Book` のデータをさらに編集・加工して利用する場合は、利用側で行った加工についても記載してください。
...
検索条件、分類による絞り込み、ISBN参照、Raw response、変換、通信、エラーの完全な仕様は[manken NDLサーチパッケージ仕様](spec.md)を参照してください。
