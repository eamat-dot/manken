...
- 表紙画像は初期実装では `summary.cover` だけを使用する
...
初期実装では、ISBN、タイトル、タイトル読み、サブタイトル、並列タイトル、巻数、
シリーズ、寄与者、出版社、出版日、表紙画像を変換する。

説明、主題、言語、ページ数、判型、物理寸法、価格、ONIX SupportingResource、
版表示は、コード値と代替書誌の欠落規則を別途確認するまで共通モデルへ設定しない。
これらはRaw responseから確認できる。
