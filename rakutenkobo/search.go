...
func validateSearchRequest(request SearchBooksRequest) (string, string, string, string, string, error) {
	title, author, publisher, freeText, excludedText := strings.TrimSpace(request.Title), strings.TrimSpace(request.Author), strings.TrimSpace(request.Publisher), strings.TrimSpace(request.FreeText), strings.TrimSpace(request.ExcludedText)
