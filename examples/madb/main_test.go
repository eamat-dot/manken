...
// TestSearchArguments_Publisher は、出版社検索を正条件として認識しISBN参照との併用を拒否することを検証する
func TestSearchArguments_Publisher(t *testing.T) {
	if !hasSearchCondition("", "", " 白泉社 ", "", "", "") {
		t.Fatal("hasSearchCondition() = false, want true")
	}
	if err := validateSearchArguments(nil, true, false); err != nil {
		t.Fatalf("validateSearchArguments(publisher search) error = %v", err)
	}
	if err := validateSearchArguments([]string{"9784088466361"}, true, true); err == nil {
		t.Fatal("validateSearchArguments(ISBN with publisher) error = nil")
	}
}
