...
func run() int {
	...
	publisher := flag.String("publisher", "", "検索する漫画の出版社名（空白区切りの全語を含む）")
	query := flag.String("query", "", "主要な書誌項目を横断する検索語（空白区切りの全語を含む）")
	excludedText := flag.String("exclude", "", "主要な書誌項目から除外する語（空白区切りのいずれかを含む本を除外）")
	from := flag.String("from", "", "出版時期の開始。YYYY、YYYY-MM、YYYY-MM-DD")
	to := flag.String("to", "", "出版時期の終了。YYYY、YYYY-MM、YYYY-MM-DD")
	...
	isbns := flag.Args()
	if err := validateSearchArguments(isbns, hasSearchCondition(*title, *author, *publisher, *query, *from, *to), searchOptionWasSet()); err != nil {
	...
	return runSearch(ctx, client, madb.SearchRequest{
		Title:     *title,
		Author:    *author,
		Publisher: *publisher,
		Query:     *query,
		Exclude:   *excludedText,
		DateFrom:  *from,
		DateTo:    *to,
		Limit:     *limit,
		Cursor:    *cursor,
...
// hasSearchCondition は、少なくとも1つの正検索条件が指定されたか判定する
func hasSearchCondition(title string, author string, publisher string, query string, from string, to string) bool {
	return strings.TrimSpace(title) != "" || strings.TrimSpace(author) != "" ||
		strings.TrimSpace(publisher) != "" || strings.TrimSpace(query) != "" ||
		strings.TrimSpace(from) != "" || strings.TrimSpace(to) != ""
}

// validateSearchArguments は、検索条件とISBN参照の組み合わせを検証する
func validateSearchArguments(isbns []string, hasSearchCondition bool, searchOptionSet bool) error {
	if len(isbns) == 0 && !hasSearchCondition {
		return errors.New("検索条件またはISBNを1件以上指定してください")
	}
	if len(isbns) != 0 && searchOptionSet {
		return errors.New("ISBNは検索用の -title、-author、-publisher、-query、-exclude、-from、-to、-limit、-cursor と併用できません")
	}
	return nil
}
...
func searchOptionWasSet() bool {
	...
	flag.Visit(func(current *flag.Flag) {
		switch current.Name {
		case "title", "author", "publisher", "query", "exclude", "from", "to", "limit", "cursor":
			wasSet = true
		}
	})
	return wasSet
...
func runSearch(ctx context.Context, client *madb.Client, request madb.SearchRequest, rawOutput string) int {
	if rawOutput == "" {
